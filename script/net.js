...
(async () => {
    try {
        try {
            ...
            try {
                if (data.version < pro.version) {
                    data.cnTimeout = pro.cnTimeout;
                    data.usTimeout = pro.usTimeout;
                    data.version = pro.version;
                    console.log("升级: V" + pro.version);
                    $persistentStore.write(JSON.stringify(data, "", 2), "KeyNetisp")
                }
            } catch (e) {
                updata = false
            }
            if (data.nw || !updata || data.version !== pro.version || typeof data.hideIP !== "boolean" || typeof data.GPT !== "boolean" || typeof data.icons !== "string" || typeof data.icolor !== "string" || typeof data.cnTimeout !== "number" || typeof data.usTimeout !== "number" || typeof data.updata !== "object") {
        ...
        re && $persistentStore.write(JSON.stringify(pro, "", 2), "KeyNetisp");
        let e = data.GPT,
            t = data.icons,
            o = data.icolor,
            a = data.hideIP,
        ...
        if (e) {
            const e = await tKey("http://chat.openai.com/cdn-cgi/trace", n);
        ...
        const g = await httpAPI();
        let y;
        const P = g.requests.slice(0, 6);
...
function HIP(e) {
    return e.replace(/(\w{1,4})(\.|\:)(\w{1,4})$/, ((e, t, o, a) => `${"∗".repeat(t.length)}.${"∗".repeat(a.length)}`))
}
