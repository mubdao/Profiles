...
(async() => {
  try {
    try {
      ...
      if (data.nw || data.version !== pro.version || typeof data.GPT !== "boolean" || typeof data.icons !== "string" || typeof data.icolor !== "string") {
    ...
    if (u.status === "success") {
      let {country:e, countryCode:t, regionName:s, query:n, city:a, org:i, isp:r, as:c, tk:p} = u;
      o = n;
    ...
    if (e) {
      const e = await tKey("http://chat.openai.com/cdn-cgi/trace", 1000);
      const t = ["CN","TW","HK","IR","KP","RU","VE","BY"];
      
      if (typeof e !== "string") {
        let {loc:s, tk:o, warp:n, ip:i} = e, 
            l = t.indexOf(s),
            r = "";
    ...
    if (f == o) {
      ...
      if (!m || h) {
        const e = await tKey(`https://api-v3.${P}${w}.cn/ip?ip=${f}`, 500);
        
        if (e.code === 0 && e.data.country === "中国") {
          let {province:t, isp:s, city:o, countryCode:n} = e.data, 
              a = e.tk;
        ...
        if ((!m || isv6) && !cn) {
          const e = await tKey(`http://ip-api.com/json/${f}?lang=zh-CN`, 1000);
          
          if (e.status === "success") {
            let {countryCode:t, country:s, city:o, org:n, tk:a, isp:l} = e;
            let c = s+" "+o;
    ...
    $done({title: a+p, content: c+n+r+zl+l, icon: t, "icon-color": s});
    
  } catch(e) {
    console.log(e.message);
    $done({title: outgpt+nodeNames, content: local+outbli+outik+outld+zl, icon: icons, "icon-color": icolor});
  }
})();
