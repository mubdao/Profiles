...
function jxh(e) { const n = e.reduce((e, n) => { const t = e.find((e) => e.name === n.name); if (t) { t.count++; if(t.count===1){t.items.push({...n,name:`${n.name}1`});} else { t.items.push({...n, name: `${n.name}${XHFGF}${t.count.toString()}`}); } } else { e.push({ name: n.name, count: 1, items: [{ ...n, name: n.name }], }); } return e; }, []); const t = n.flatMap((e) => e.items); e.splice(0, e.length, ...t); return e;};
function oneP(e) { const t = e.reduce((e, t) => { const n = t.name.replace(/[^A-Za-z0-9\u00C0-\u017F\u4E00-\u9FFF]+\d+$/, ""); if (!e[n]) { e[n] = []; } e[n].push(t); return e; }, {}); for (const e in t) { if (t[e].length === 1) { const n = t[e][0]; n.name = e; } } return e; };
