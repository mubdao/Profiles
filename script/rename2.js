...
function jxh(e) { const n = e.reduce((e, n) => { const t = e.find((e) => e.name === n.name); if (t) { t.count++; if (t.count === 1) { t.items.push({...n, name: n.name}); } else { t.items.push({...n, name: `${n.name}${XHFGF}${t.count.toString()}`}); } } else { e.push({ name: n.name, count: 1, items: [{ ...n, name: n.name }], }); } return e; }, []); const t = n.flatMap((e) => e.items); e.splice(0, e.length, ...t); return e;};
