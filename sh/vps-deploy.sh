...
read -p "④ 网卡名 [默认 auto 自动检测]: " INTERFACE
...
# --- 写入定时任务（只保留每天00:00重置今日基准）---
echo -e "${GREEN}[4/4] 正在配置定时任务...${NC}"
(crontab -l 2>/dev/null | grep -v "vps_monitor.sh"; \
echo "0 0 * * * TZ='Asia/Shanghai' /bin/bash /root/505/vps_monitor.sh --daily-reset >/dev/null 2>&1"; \
...
echo -e "${GREEN}========================================${NC}"
echo -e "发送 /report 给 Bot 随时查看流量"
echo -e "今日消耗每天 00:00 自动重置"
echo -e "${GREEN}========================================${NC}\n"
